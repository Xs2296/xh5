  ...
  methods: {
    getAuthInfo() {
      Profile.getAuthInfo().then((res) => {
        this.form.front_img = res.data.front_img
        this.form.back_img = res.data.back_img
    ...
    topAuth() {
      let data = this.form;
      if (!data.front_img) {
        this.$toast(this.$t('auth.b5'));
        return;
      }
      if (!data.back_img) {
        this.$toast(this.$t('auth.b6'));
        return;
      }
