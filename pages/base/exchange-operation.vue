<template>
	<view class="d-flex flex-col">
		...
		<!-- 左侧的弹窗 -->
		<van-popup :show="symbolListShow" @close="symbolListShow = false" close-on-popstate position="left" custom-style="height:100%;width:70%">
...
export default {
	...
	mounted() {
		let _this = this;
		uni.$off('changeData-0');
		uni.$on('changeData-0', function(res) {
			console.log(res)
...
.title-group {
	width: 100%;
	margin-top: calc(20em * 0.071);
	margin-bottom: calc(10em * 0.071);
	font-size: calc(14em * 0.071);
	display: flex;
	flex-direction: row;
