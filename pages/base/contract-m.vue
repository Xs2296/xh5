<template>
	<view id="box-page" class="d-flex flex-col">
		...
		<view class="box-c-cont-1">
			<view v-if="activeCoin != undefined" class="num" :class="activeCoin.increase < 0 ? 'color-sell' : 'color-buy'">{{ numFormat(activeCoin.price) }}</view>
			<!-- <view>≈ €20,339.07</view> -->
		</view>
		...
		<view class="button">
			<radio-group class="uni-list align" @change="radioChange">
				<label class="label">
					<view><radio :id="radioList[0].id" :value="radioList[0].id" :checked="true" @click="at_time = 1" /></view>
				...
				<label class="label">
					<view><radio :value="radioList[1].id" :id="radioList[1].id" :checked="false" @click="at_time = 2" /></view>
					<label class="label-2-text">
		...
		<view class="box-c-cont" style="margin-bottom: calc(60em * 0.0625); position: relative;">
			...
			<van-tabs :active="tabs_active" class="box-tabs records" @change="tabsActiveEv">
				...
				<van-tab :title="$t('expand.预约记录')"><!-- 交易记录 --></van-tab>
			</van-tabs>
			...
			<view class="box-table" v-if="tabs_active == 1 || tabs_active == 0">
				<view class="box-log-item" v-for="(item, i) in orderLog" :key="i">
					...
					<view class="box-item-info d-flex">
						...
						<view class="value" v-if="tabs_active == 1 || tabs_active == 0">{{ numFormat(parseFloat(item.buy_price).toFixed(2)) }} {{ symbolRight }}</view>
					...
					<view class="box-item-info d-flex">
						...
						<view class="value" v-if="tabs_active == 1 || tabs_active == 0">{{ numFormat(parseFloat(item.buy_num).toFixed(2)) }} {{ symbolRight }}</view>
						<view class="value" v-if="tabs_active == 2">{{ item.buy_time }}</view>
					</view>
					<view class="box-item-info d-flex">
						<view class="title" v-if="tabs_active == 2">{{ $t('assets.d0') }}</view>
						<view class="value" v-if="tabs_active == 2">{{ item.status == 1 ? $t('expand.定时中') : item.status == 2 ? $t('otc.c9') : $t('expand.已取消') }}</view>
					</view>
					...
					<view class="box-item-info d-flex" v-if="tabs_active == 1">
						...
						<view class="value" :class="item.order_result == 2 ? 'text-red' : 'text-green'">
							{{ item.order_result == 2 ? '-' : '' }}{{ numFormat(parseFloat(item.profits).toFixed(2)) }} {{ symbolRight }}
						</view>
					</view>
							...
							<view class="box-tabs_content">
								<table class="w-max a">
									...
									<tbody class="table-list">
										<tr class="link-active" v-for="(item, i) in collect" :key="i" @click="checkSymbol(item)">
							...
							<view class="box-tabs_content">
								<table class="w-max a">
									...
									<tbody class="table-list">
										...
										<tr class="link-active" v-for="(item, i) in marketList[0].marketInfoList" :key="i" @click="checkSymbol(item)">
											<td class="p-l-md p-y-xs rounded-tl-xs rounded-bl-xs">
		...
		<view class="datetime" v-if="isTimeShow == true">
			<van-datetime-picker :value="currentDate" @change="change" type="datetime" :min-date="minDate" :max-date="maxDate" @cancel="cancel" @confirm="confirm" />
		</view>
	</view>
</template>
...
export default {
	...
	created() {
		...
		this.time = myDate.getFullYear() + '/' + (myDate.getMonth() * 1 + 1) + '/' + myDate.getDate() + ' ' + myDate.getHours() + ':' + myDate.getMinutes();
	},
	mounted() {
		let _this = this;
	...
	methods: {
		...
		change(e) {
			...
			this.time = e.detail.getValues()[0] + '/' + e.detail.getValues()[1] + '/' + e.detail.getValues()[2] + ' ' + e.detail.getValues()[3] + ':' + e.detail.getValues()[4];
		},
		cancel() {
			// 取消
			// console.log('取消');
			...
			Hangqing.getOrderLog(para).then(res => {
				...
				if (this.tabs_active == 0) {
					for (let i = 0; i < this.orderLog.length; i++) {
						...
						this.orderLog[i]['time'] = parseInt(this.orderLog[i].second) - parseInt(parseInt(d1 - d2) / 1000); //parseInt(this.orderLog[i].second);
